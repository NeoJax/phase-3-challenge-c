...
function grabBookings() {
  return db.any('SELECT * FROM bookings JOIN guests ON guests.id = bookings.guest_id JOIN rooms ON rooms.id = bookings.room_id WHERE check_in > now() ORDER BY check_in')
    .catch((err) => {
      console.log(err);
    });
}
...
function grabRoomBookings(roomNum) {
  return db.any('SELECT * FROM bookings JOIN guests ON guests.id = bookings.guest_id JOIN rooms ON rooms.id = bookings.room_id WHERE room_num=$1 AND check_in > now() ORDER BY check_in', [roomNum])
    .catch((err) => {
      console.log(err);
    });
}
